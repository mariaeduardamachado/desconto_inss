function mostrarRelatorio() {
    ...
    faixasSalariais.forEach(faixa => {
        report[faixa.faixa] = funcionarios.filter(funcionario => 
            funcionario.salario >= faixa.min && funcionario.salario <= faixa.max
        ).length;
    });

    const reportDiv = document.getElementById('report');
    reportDiv.innerHTML = '<h2>Relatório:</h2>';
    for (const faixa in report) {
        reportDiv.innerHTML += `<p>${faixa}: ${report[faixa]} funcionários</p>`;
    }
    ...
    const grafico = new Chart(graficoCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: faixasSalariais.map(faixa => report[faixa.faixa]),
                backgroundColor: [
                    'rgba(255, 99, 132, 0.2)',
                    'rgba(54, 162, 235, 0.2)',
                    'rgba(255, 206, 86, 0.2)',
